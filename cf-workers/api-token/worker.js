// cf-workers/api-token/worker.js
/* ===========================================
   INPI Token API – Presale + Early Claim
   KV: INPI_CONFIG, INPI_PRESALE, INPI_CLAIMS
   ENV (optional): RPC_URL, HELIUS_API_KEY, PRESALE_MIN_USDC, PRESALE_MAX_USDC, RECONCILE_KEY
   =========================================== */
...
/* ---- Binding-Helper: unterstützt auch legacy CONFIG, aber bevorzugt INPI_CONFIG ---- */
function CFG_KV(env) { return env.INPI_CONFIG ?? env.CONFIG; }
    ...
    try {
      ...
      if (req.method === "POST" && p === "/api/token/presale/intent") {
        ...
        return J({
          ...
          qr_contribute: contribute,
          qr_claim_now: claimNow,
          airdrop_bonus_bps: toNum(cfg.airdrop_bonus_bps, 600),
          updated_at: Date.now()
        });
      }
...
async function readCfg(env){
  ...
  const out = {};
  await Promise.all(keys.map(async k => (out[k] = await KV.get(k))));
  return out;
}
...
async function getRpc(env){
  const KV = CFG_KV(env);
  const fromCfg = await KV.get("public_rpc_url").catch(()=>null);
